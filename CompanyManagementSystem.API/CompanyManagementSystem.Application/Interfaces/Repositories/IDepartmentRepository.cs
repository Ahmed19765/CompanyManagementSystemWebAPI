...
namespace CompanyManagementSystem.Application.Interfaces.Repositories
{
    public interface IDepartmentRepository
    {
        ...
        Task<bool> ExistsByNameInCompanyAsync(int companyId, string departmentName, int? excludedDepartmentId = null);
        Task SaveChangesAsync();

        // ── Query methods ──────────────────────────────────────────────────────────

        /// <summary>Returns all departments (with their Teams) for a given company.</summary>
        Task<IEnumerable<Department>> GetAllByCompanyIdAsync(int companyId);
        ...
        /// </summary>
        Task DeleteAllByCompanyIdAsync(int companyId);
